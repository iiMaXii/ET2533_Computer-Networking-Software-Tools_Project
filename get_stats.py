...
usage: get_stats.py [option]
Options:
-c , --count-maph
       Create a table for the most active host pairs
-h , --help
...
for argument in sys.argv[1:]:
	if argument == '-c' or argument == '--count-maph':
		count_mahp = True
	else:
		print('usage: {} [option]'.format(sys.argv[0]))
		print('Options:')
		print('-c , --count-maph')
		print('       Create a table for the most active host pairs')
		print('-h , --help')
		print('       Print the usage for the script')
		sys.exit()
...
for filename in sorted(glob.glob("stats_archive[0-9].txt")):
	if count_mahp:
		print('    \\verb|{}| '.format(get_archive_name(filename)), end='')
	...
	if count_mahp:
		mahp_occurences_iter = iter(collections.Counter(mahp_list).items())

		mahp_first = next(mahp_occurences_iter)
		...
		print('    \\hline')
		for stat_type, data in stats[filename].items():
